...
class MerchantQrView extends GetView<MerchantQrController> {
  const MerchantQrView({super.key});

  @override
  Widget build(BuildContext context) {
    final c = Get.put(MerchantQrController());
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            AdBannerWidget(slot: c.adService.overlay),
          ],
        ),
      ),
    );
  ...
  Widget _fieldByIndex(BuildContext context, MerchantQrController c, int indx) {
    ...
    if (indx == 62) {
      return MerchantAdditionalFieldTile(controller: c, spec: o);
    }
    if (indx == 63) {
      return const Center(
    ...
    if (indx == 64) {
      return MerchantLanguageFieldTile(controller: c, spec: o);
    }
  ...
  Widget _commonField(MerchantQrController c, int indx) {
    final o = c.getSpec(indx);
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: GlassTheme.inputDecoration(
          labelText: '${o.Id}-${o.Name}',
        ),
      ),
    );
  }

  Widget _pointOfInitiation(MerchantQrController c) {
  ...
  Widget _tipIndicator(MerchantQrController c) {
    return Obx(
      () => Padding(
        padding: const EdgeInsets.fromLTRB(12, 8, 12, 4),
        child: Column(
  ...
  Widget _merchantSelector(MerchantQrController c) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Padding(
        ...
        Padding(
          ...
          child: Obx(
            () => AppDropdown<String>(
              value: c.selectedMerchant.value,
              ...
              onChanged: (v) {
                if (v != null) {
                  c.selectedMerchant.value = v;
                  c.addMerchantField(v);
                }
              },
            ),
          ),
        ),
        Obx(() => Column(children: c.merchantInformation.toList())),
      ],
    );
  }

  Widget _unreservedSelector(MerchantQrController c) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Padding(
